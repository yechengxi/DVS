...
class LabSequenceFolder(data.Dataset):
    ...
    def crawl_folders(self, sequence_length):
        ...
        for scene in self.scenes:
            ...
            for i in range(demi_length, len(imgs) - demi_length):
                sample = {'intrinsics': intrinsics, 'tgt': imgs[i], 'ref_imgs': [], 'depth': self.depths[i],'D':distortion}
                for j in shifts:
                    sample['ref_imgs'].append(imgs[i + j])
                if self.train or os.path.exists(self.depths[i]) or (not self.gt):
                    sequence_set.append(sample)
    ...
    def __getitem__(self, index):
        ...
        median = np.median(depth[depth > 10])
        if np.isnan(median):
            depth[:] = 2500
        else:
            depth[depth < (median * .8)] = 2500
        depth = depth.astype(np.float32) / 2500*255
