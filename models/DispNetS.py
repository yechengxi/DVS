...
def downsample_conv(in_planes, out_planes, kernel_size=3,normalization=''):
    if normalization =='gn':
        return nn.Sequential(
            nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=2, padding=(kernel_size - 1) // 2),
            GroupNorm(num_features=out_planes),
...
def conv(in_planes, out_planes,normalization=''):
    if normalization =='gn':
        return nn.Sequential(
...
def upconv(in_planes, out_planes,normalization=''):
    if normalization =='gn':
        return nn.Sequential(
