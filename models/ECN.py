...
class ECN_PixelPose(nn.Module):
    def __init__(self, input_size, nb_ref_imgs=2, init_planes=16, scale_factor=0.5, growth_rate=16,
                 final_map_size=1, output_exp=False,output_pixel_pose=False,
                 norm_type='gn'):
        ...
        self.pred_planes = 6+nb_ref_imgs
    ...
    def forward(self, target_image, ref_imgs):
        ...
        pixel_pose = [0.01 *predicts[i][:, :6] for i in range(self.predicts)]

        if self.training:
            return exps, pixel_pose
        else:
            return exps[0], pixel_pose[0]




class ECN_DispPose(nn.Module):
    def __init__(self, input_size, nb_ref_imgs=4, init_planes=32, scale_factor=0.5, growth_rate=32,
                 final_map_size=1, output_exp=False, output_disp=True, alpha=10, beta=0.01,
                 norm_type='gn'):
        super(ECN_DispPose, self).__init__()
        self.scale_factor = scale_factor
        self.final_map_size = final_map_size
        self.encoding_layers = nn.ModuleList()
        self.decoding_layers = nn.ModuleList()
        ...
        self.alpha = alpha
        self.beta = beta
        self.pred_planes = nb_ref_imgs + 1
    ...
    def forward(self, input):
        ...
        pose = self.pose_pred(out)
        pose = pose.mean(3).mean(2)
        pose = 0.001 * pose.view(pose.size(0), 1, 6)
        ...
        if self.training:
            return disps, exps, pose
        else:
            return disps[0], exps[0], pose
